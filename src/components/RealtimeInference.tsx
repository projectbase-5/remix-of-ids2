...
const RealtimeInference: React.FC<RealtimeInferenceProps> = ({ activeModel }) => {
  ...
  // Convert NetworkEvent to MLFeatures
  const convertNetworkEventToFeatures = (event: NetworkEvent): MLFeatures => {
    return {
      duration: Math.random() * 1000, // Simulated duration
      protocol_type: event.protocol.toLowerCase(),
      service: 'http', // Simplified for demo
      flag: 'SF', // Simulated flag
      src_bytes: event.packetSize,
      dst_bytes: Math.floor(event.packetSize * 0.8),
      land: event.sourceIP === event.destinationIP ? 1 : 0,
      wrong_fragment: 0,
      urgent: 0,
      hot: Math.random() > 0.9 ? 1 : 0,
      num_failed_logins: 0,
      logged_in: 1,
      num_compromised: 0,
      root_shell: 0,
